...
#ifdef FP_IMPLEMENTATION
{
	...
	uint8_t* p = (uint8_t*)FP_ALLOCATION_FUNCTION(NULL, size);
	p += FPDA_HEADER_SIZE;
	if(!p) return 0;
...
inline static bool is_fpda(void* da) FP_NOEXCEPT { return __fpda_header(da)->h.magic == FP_DYNARRAY_MAGIC_NUMBER; }
