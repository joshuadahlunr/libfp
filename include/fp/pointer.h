...
#define FP_EXPAND(VAL) FP_DO_EXPAND(VAL)
	...
	#include <type_traits>
	template<typename T>
	using fp_type_of_t = std::remove_reference_tT>;
	...
	#include <type_traits>
	template<typename T>
	using fp_type_of_t = std::remove_reference<T>::type;
	#define FP_TYPE_OF(x) fp_type_of_t<decltype(x)>
	template<typename T>
	using fp_type_of_remove_pointer_t = std::remove_pointer<fp_type_of_t<T>>::type;
	#define FP_TYPE_OF_REMOVE_POINTER(x) fp_type_of_remove_pointer_t<decltype(x)>
#endif
...
#if defined __cplusplus && __cpp_lib_span >= 202002L
	#include <span>
#endif
...
#else
	return std::bit_cast<__FatPointerHeader*>(p);
#endif
}
...
inline static void* __fp_malloc(size_t size) FP_NOEXCEPT { return __fp_alloc(NULL, size); }
...
inline static void* __fp_clone(const void* ptr, size_t type_size) FP_NOEXCEPT {
	...
	auto out = __fp_malloc(size * type_size);
	__fp_header(out)->size = size;
...
#define fp_void_view_literal(data, size) (fp_void_view{(void*)(data), (size)})
...
#define fp_view_back(type, view) ((type*)__fp_view_back((view), sizeof(type)))
#define fp_view_end(type, view) (fp_view_back(type, (view)) + 1)
...
FP_CONSTEXPR inline static bool fp_view_compare_equal(const fp_void_view a, const fp_void_view b) FP_NOEXCEPT {
